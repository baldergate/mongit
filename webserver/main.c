...
int main(int argc , char **argv){
	...
	while((socket_client = accept(socket_serveur , NULL , NULL))!=-1){
	  ...
	  if(fork()==0) {
	    ...
	    fprintf(f,"%s",message_bienvenue);
	    /*  write(socket_client , message_bienvenue , strlen(message_bienvenue));*/
	    while(fgets(message_client,sizeof(message_client),f)!=NULL/*(taille=read(socket_client,  message_client, 80))!=0*/){
	     
	      if (taille == -1)
		{
		  perror("rien");	/* traitement d'erreur */
		}
	       fprintf(f,"<mongit> %s",message_client);
